...
export default function DashboardPage() {
  ...
  const lineData = (() => {
    ...
    requests.forEach((r) => {
      const month = new Date(r.requested_at).toLocaleString("default", {
        month: "short",
      });
      monthMap[month] = (monthMap[month] || 0) + 1;
    });
    return Object.entries(monthMap).map(([month, requests]) => ({
      month,
      requests,
    }));
  })();
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {recentRequests.map((req) => (
                <tr
                  onClick={() =>
                    router.push(`/dashboard/staff/vehicle-request/${req.id}`)
                  }
                  key={req.id}
                  className="border-t cursor-pointer border-gray-100 hover:bg-gray-50"
                >
